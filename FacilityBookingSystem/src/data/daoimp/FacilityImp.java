...
public class FacilityImp extends BaseConnection implements FacilityDAO{
    ...
    public Facility getFacility(String facID) throws NotFoundException, SQLException {
    ...
    public void updateFacility(Facility facility) throws NotFoundException, SQLException {
    ...
    public void deleteFacility(Facility facility) throws NotFoundException, SQLException {
          if (facility.getFacID() == null) {
    ...
    public List<Facility> searchMatching(Facility facility) throws SQLException {
          ...
          if (facility.getFacID() != null) {
              ...
              sql.append("AND FacID LIKE '").append(facility.getFacID()).append("%' ");
          }

          if (facility.getFacName() != null) {
              if (first) { first = false; }
              sql.append("AND FacName LIKE '").append(facility.getFacName()).append("%' ");
          }

          if (facility.getFacUsage() != null) {
              if (first) { first = false; }
              sql.append("AND FacUsage LIKE '").append(facility.getFacUsage()).append("%' ");
          }

          if (facility.getTypeID() != 0) {
              if (first) { first = false; }
              sql.append("AND TypeID = ").append(facility.getTypeID()).append(" ");
          }
    ...
    protected List<Facility> listQuery(PreparedStatement stmt) throws SQLException {
